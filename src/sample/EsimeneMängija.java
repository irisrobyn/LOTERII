...
public class EsimeneMängija {

    @FXML
    private TextField esimeneNimi;
    @FXML
    private TextField piletiteNumbrid;
    @FXML
    ...
    public void mängi(javafx.event.ActionEvent actionEvent) throws IOException {
        ...
        mängijad.add(saanMängija(piletiteNumbrid, esimeneNimi));
        loto.mängime(mängijad);

        if (loto.kasVõitis()) {
            DecimalFormat df = new DecimalFormat("0.00");

            Võitjad.setText("Palju õnne!"  + loto.võitjad() + " võitis " + df.format((loto.piletiteSumma/loto.võitjad.size())*0.95) + " eurot!");
